...
myFolder = 'Raphael-jpg/1'
file_name =  '5_1.jpg';
filePattern = fullfile(myFolder, file_name)

%x = histeq(imreadbw(filePattern));
x = imreadbw(filePattern);
...
%%%feature extraction
feature = sum(sum(S_mat,2),3);   %%sum every little graphWop = wavelet_factory_2d(size(x));

file2npy=strrep(file_name,'.jpg','.npy');
writeNPY(feature, file2npy);
